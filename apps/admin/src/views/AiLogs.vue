<template>
  <div>
    ...
    <div class="toolbar">
      <el-select v-model="query.toolName" placeholder="全部工具" clearable style="width: 200px" @change="search">
...
const { rows, total, loading, query, load, search, onPage } = useTable(
  (q, signal) => api.toolCalls(q, signal),
  { toolName: '', status: '', page: 1, pageSize: 20 },
);
...
async function openDetail(row: Record<string, any>) {
  ...
  await runAction(async () => {
    await router.push('/ai/conversations?id=' + row.conversationId);
  }, '打开会话失败');
}
