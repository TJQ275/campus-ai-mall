...
| 智能客服与售后 | 知识库 **RAG（召回 20 → Python 重排 → 精选 4）**，回答带引用来源，可代提交售后 | 关键词检索 + 同义词扩展 + 本地重排 |
...
大多数 AI 助手项目只做到「能聊」，缺的是上线之后怎么管。这里补了两块：

**① AI 评测**（`pnpm ai:eval`）—— 57 条固定用例，覆盖导购 / 政策 RAG / 交易 / 商家 / 边界安全，
输出工具命中率、答案要点命中率、克制率、写操作确认率、幻觉与提示词泄漏数、平均延迟，
并和上次基线对比。判定逻辑是纯函数，自带 10 条单元测试。

> 它不是摆设：第一次跑导购场景只有 **74%**，抓出 4 个真实缺陷（回复重复、轮次用尽返回空回复、
> 整段英文作答、相同参数循环检索）。修完复跑 **100%**。

**② AI 成本管控**（后台「AI 成本」页）—— 每次模型调用按「调用」粒度记一条用量，
按微元精确折算费用，支持按天 / 模型 / 场景 / 用户聚合，并可设日预算；
超预算后 AI 自动降级到本地模式，不再产生费用。

> 顺带修掉一个真实 bug：原先只在最终回复上记 token，**中间的工具轮次全丢了** ——
> 一次 3 轮工具的对话实际付了 3 次钱，账上只记 1 次。现在实测正确记 3 条。

**③ Agent 工作流与作答前校验** —— 把「一问一答」拆成有名字、可观测的阶段：
`理解意图 → 规划步骤 → 执行工具 → 校验回答 → 生成回复`，阶段事件随 SSE 推给前端。

> **校验阶段不是摆设。** 它用一组规则在**发给用户之前**拦掉问题回答，最关键是 R1：
> 「声称做了某个写操作，但本轮既没调用写工具、也没有生成待确认动作」——
> 这正是评测抓出的那个事故（模型说「已加入购物车」而工具链里没有 `add_to_cart`）。
> 命中阻断规则会自动带着纠正指令重写一次；重写仍不通过就返回一句诚实的兜底说明，
> **宁可让用户觉得「没办成」，也不能让他以为「办成了」**。

**④ RAG 的「重排」段用独立 Python 微服务**（[services/rerank](services/rerank/README.md)）：
BM25（中文单字+双字）+ RRF 融合召回排名，零重依赖、不下载模型权重。
召回放宽到 20 条再精选 4 条，而不是直接召回 4 条 —— 后者会让最相关的那条根本没进候选。
服务挂了自动跳过重排，检索照常可用并在后台标红。

详见 [docs/10-AI评测与成本.md](docs/10-AI评测与成本.md)。
