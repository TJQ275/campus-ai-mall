...
| 阶段 | 内容 | 状态 |
|---|---|---|
| P1 | monorepo / 数据库（29 张表）/ 双驱动连接 / API 文档 | ✅ |
| P1 | 认证、商品检索、购物车、地址、订单支付、售后、钱包 | ✅ |
| P2 | **AI 骨架：Provider 抽象、8 个工具、Agent 主循环、SSE、二次确认、无 Key 降级** | ✅ |
| P3 | 客服 RAG、拍照找同款、评论摘要、个性化推荐理由 | ⏳ 下一步 |
| P4 | 商家侧 AI、Vue3 管理后台、小程序端、部署文档与演示脚本 | ⏳ |

**已验证的端到端链路**：管理端登录 → 小程序登录 → 商品检索 → 加地址 → 充值 → 加购 → 下单 → 余额支付 → 发货 → 收货 → 申请售后 → 审核退款入账与库存回滚 → 数据概览；AI 侧「想吃辣的，20 元以内」→ 工具调用 → 5 张商品卡片 → 「把第一个加购」→ 待确认 → 确认后写入购物车。

演示账号：管理端 `admin / admin123`；小程序端任意 code 即可登录（未配置 `WX_APPID` 时后端走开发模式，`code` 直接映射为 openid）。

**没有大模型 Key 也能完整演示**：`LLM_API_KEY` 留空时自动使用内置的规则引擎（MockProvider），会解析预算 / 辣度 / 品类 / 课程 / 订单意图并走与真实模型完全相同的工具调用链路，`GET /api/ai/status` 可以看到当前模式。
