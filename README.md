...
| P4 | 部署文档、演示脚本、README 收尾与全链路验收 | ⏳ 下一步 |
...
pnpm dev:server     # 后端      http://localhost:3100/api
pnpm dev:admin      # 管理后台  http://localhost:5173   (admin / admin123)
pnpm dev:miniapp    # 小程序    用微信开发者工具打开 apps/miniapp
§§§

小程序需要勾选「详情 → 本地设置 → 不校验合法域名」；不需要真实 AppID，后端未配置 `WX_APPID` 时会走开发模式自动建号。

## 自动化验证

§§§bash
pnpm test:miniapp   # 22 项断言：小程序 API 层真实打后端 + SSE 跨 chunk 汉字解码
§§§

**已验证链路**：

- 电商闭环：登录 → 检索 → 加购 → 下单 → 余额支付 → 发货 → 收货 → 售后 → 审核退款（入账 + 库存回滚）→ 数据概览
- AI 导购：「想吃辣的，20 元以内」→ 工具调用 → 商品卡片 → 「把第一个加购」→ 待确认 → 确认写入购物车
- 客服 RAG：5 个政策问题全部命中正确条款并输出「根据《…》」引用
- 找同款：扫码 ISBN 精确命中；无视觉模型时明确降级并给出替代路径
- 评论摘要：真实评价 → 评分分布 + 优点 + 槽点（模板路径标注 model=template）
- 推荐：每件商品带可解释理由，召回链路 UserCF → 画像补齐 → 热销兜底
- 商家侧：「最近生意怎么样」→ 真实 KPI；「哪些商品卖得最好」→ 热销排行；「帮这款写文案」→ 基于真实字段的文案
- 管理后台：数据概览 / AI 调用日志 / AI 会话回放 / 知识库 / 商品 / 订单 / 售后 / 用户
- 小程序：14 个页面结构校验通过，SSE 流式对话与写操作确认可用
